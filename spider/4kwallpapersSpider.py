...
async def parse_page(session: aiohttp.ClientSession, url: str) -> List[Dict[str, str]]:
    ...
    for item in items:
        ...
        if pic_html:
            ...
            pic_url = MAIN_URL + psoup.find('span', class_="res-ttl").find_all('a')[-1]['href']
...
async def download_pic(session: aiohttp.ClientSession, url: Dict[str, str]) -> None:
    try:
        async with session.get(url['url'], headers=HEADERS, timeout=aiohttp.ClientTimeout(total=TIMEOUT)) as response:
            if response.status == 200:
                file_path = BASE_DOWNLOAD / f"{url['title']}.jpg"
                async with aiofiles.open(file_path, mode='wb') as f:
                    await f.write(await response.read())
            else:
                logger.error(f"Failed to download {url['url']}. Status: {response.status}")
    except asyncio.TimeoutError:
        logger.error(f"Timeout error while downloading {url['url']}")
    except Exception as e:
        logger.error(f"Error downloading {url['url']}: {str(e)}")

async def process_page(session: aiohttp.ClientSession, url: str, semaphore: asyncio.Semaphore) -> None:
...
async def main() -> None:
    ...
    async with aiohttp.ClientSession() as session:
        tasks = [
            process_page(session, f'{MAIN_URL}/?page={p}' if p != 1 else MAIN_URL, semaphore)
            for p in range(1, 10)
        ]
